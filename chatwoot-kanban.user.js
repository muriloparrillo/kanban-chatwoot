...
// @version      2.0.0
// @description  Adiciona o Kanban CRM como item nativo no menu lateral do Chatwoot
...
(function () {
  ...
  function fetchFunnels(cb) {
    ...
    .catch(function(err) { log('fetch error:', err); cb([]); });
  }
   ...
   * Múltiplos seletores para Chatwoot v2, v3 e variantes Tailwind.
   * ──────────────────────────────────────────────────────────────────── */
  function findNavContainer() {
    ...
    for (var i = 0; i < selectors.length; i++) {
      ...
      if (el) {
        log('container via:', selectors[i]);
        return el;
      }
    }
    // Fallback: encontrar pelo link de conversas
    var convLink =
  ...
  function inject() {
    ...
    if (!container) {
      if (retryCount <= 3 || retryCount % 10 === 0) {
        log('sidebar não encontrado, tentativa', retryCount, '| aside:', !!document.querySelector('aside'));
      }
      return;
    }
    ...
    log('\u2705 injetado! container:', container.tagName, container.className || '(sem classe)');
  }

  /* ─── OBSERVER: re-injeta após navegação SPA ──────────────────────── */
  new MutationObserver(function() {
    if (!document.getElementById('crm-sidebar-group') && findNavContainer()) {
      inject();
    }
  ...
  var interval = setInterval(function() {
    ...
    if (retryCount >= MAX_RETRIES) {
      ...
      log('\u26A0\uFE0F timeout após 30s. Inspecione o DOM e abra uma issue.');
    }
  }, 500);

  inject(); // Tentativa imediata
